...
describe("GET /members/geo-cluster", () => {
  it("rejects an unauthenticated request", async () => {
    const res = await app.inject({ method: "GET", url: "/members/geo-cluster" });
  ...
  it("rejects an invalid status filter", async () => {
    ...
    const res = await app.inject({
      method: "GET",
      url: "/members/geo-cluster?status=bogus",
      headers: authedHeaders(token),
    });
